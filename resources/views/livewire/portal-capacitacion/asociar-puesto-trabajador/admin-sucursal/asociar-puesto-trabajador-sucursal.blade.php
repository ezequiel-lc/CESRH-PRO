            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-4">
                @foreach($opciones as $opcion)
                    <div class="p-5 border rounded-xl shadow-lg hover:bg-gray-100 cursor-pointer transform hover:scale-105 transition duration-300 ease-in-out flex flex-col items-center text-center">
                        ...
                        <a class="text-lg font-semibold text-indigo-600 hover:text-indigo-800" href='{{ route("asignarPerfilPuestoSucursal", ["id" => Crypt::encrypt($opcion->id), "tipoUsuario" => $tipo_seleccionado]) }}'>{{ $opcion->usuarios->name ?? 'Sin nombre' }}</a>
