<div class="h-full bg-gray-200 p-8">
    ...
    <div class="flex relative rounded-md w-full max-w-3xl mx-auto">
        <input type="text" wire:model.debounce.500ms="search" placeholder="Nombre de usuario o RFC"
            ...
            <div class="w-full mx-auto bg-white shadow-xl rounded-lg text-gray-900 p-4">
                ...
                <div class="text-center mt-2">
                    ...
                    <p class="text-gray-500">
                        Reg Patronal: {{ $registros_patronales[$instructor->registro_patronal_id]->registro_patronal ?? 'Sin Departamento' }} 
                    </p>
                    <p class="text-gray-500">
                        Empresa: {{ $emp[$usuarios[$instructor->user_id]->empresa_id]->nombre ?? 'Sin Empresa' }}
                    </p>                    
                    <p class="text-gray-500">
                        Sucursal: {{ $suc[$usuarios[$instructor->user_id]->sucursal_id]->nombre_sucursal ?? 'Sin Sucursal' }}
                    </p>
                    <p class="text-gray-500">
                        Departamento: {{ $depa[$usuarios[$instructor->user_id]->departamento_id]->nombre_departamento ?? 'Sin Departamento' }}
                    </p>
                    <p class="text-gray-500">
                        Puesto: {{ $puest[$usuarios[$instructor->user_id]->puesto_id]->nombre_puesto?? 'Sin Puesto' }}
                    </p>
                </div>
